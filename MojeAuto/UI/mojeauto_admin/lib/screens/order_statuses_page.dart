...
class OrderStatusesPage extends StatefulWidget {
  const OrderStatusesPage({super.key});

  @override
  State<OrderStatusesPage> createState() => _OrderStatusesPageState();
}

class _OrderStatusesPageState extends State<OrderStatusesPage> {
  ...
  Future<void> _addStatus() async {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Status narudžbe uspješno dodan."),
      ...
      final errorMessage = extractErrorMessage(
        response,
        fallback: "Greška pri dodavanju statusa narudžbe.",
      );
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _updateStatus(int id) async {
    ...
    if (response.statusCode == 200 || response.statusCode == 204) {
      ...
      final errorMessage = extractErrorMessage(
        response,
        fallback: "Greška pri ažuriranju statusa narudžbe.",
      );
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(errorMessage), backgroundColor: Colors.red),
      );
  ...
  Future<void> _deleteStatusConfirmed(int id) async {
    ...
    if (response.statusCode == 204) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text("Status narudžbe obrisan."),
          backgroundColor: Colors.green,
        ),
      );
    } else {
      final errorMessage = extractErrorMessage(
        response,
        fallback: "Greška pri brisanju statusa narudžbe.",
      );
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(errorMessage), backgroundColor: Colors.red),
      );
          ...
          actions: [
            ...
            TextButton(
              ...
              onPressed: () {
                ...
                _deleteStatusConfirmed(status['orderStatusId']);
              },
            ),
          ],
        );
            ...
            child: Form(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    editingId != null
                        ? "Uredi status narudžbe"
                        : "Dodaj status narudžbe",
            ...
            children: [
              ...
              Expanded(
                child: isLoading
                    ...
                    ? const Center(
                        child: Text(
                          "Nema dostupnih statusa narudžbi.",
                              ...
                              children: [
                                ...
                                PopupMenuButton<String>(
                                  ...
                                  onSelected: (value) {
                                    if (value == 'edit') {
                                      setState(() {
                                        editingId = status['orderStatusId'];
